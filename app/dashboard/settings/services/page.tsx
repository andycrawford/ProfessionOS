...
function StatusBadge({ status }: { status: ServiceStatus }) {
  const icons: Record<ServiceStatus, React.ReactNode> = {
    ...
    polling: <Loader2 size={12} className={styles.spinner} aria-hidden="true" />,
  };
  ...
  return (
    <span className={`${styles.statusBadge} ${styles[status]}`} aria-label={`Status: ${labels[status]}`}>
      {icons[status]}
      {labels[status]}
    </span>
  );
  ...
  useEffect(() => {
    fetch("/api/services")
      .then((res) => {
        ...
        return res.json() as Promise<ConnectedService[]>;
      })
      .then(setServices)
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <Link href="/dashboard/settings/services/new" className={styles.addButton}>
      ...
      {!loading && !error && services.length === 0 && (
        <div className={styles.empty}>
          <Plug size={32} className={styles.emptyIcon} aria-hidden="true" />
          <p className={styles.emptyTitle}>No services connected yet</p>
          <p className={styles.emptyBody}>
            Connect a service to start monitoring emails, calendar events, tasks, and more.
          </p>
          <Link href="/dashboard/settings/services/new" className={styles.addButton}>
            <Plus size={14} aria-hidden="true" />
            ...
            <li key={svc.id} className={styles.card}>
              ...
              <div className={styles.info}>
                ...
                <div className={styles.meta}>
                  {svc.itemCount} items · Last sync: {formatLastSync(svc.lastSyncedAt)}
                </div>
              </div>
